...
import type {
  DirectoryContent,
  DirectoryContentBatch,
} from "@shared/types/apiTypes";

export const directoryApi = {
  // TODO: unused
  fetchDirectory: async (dirId: string): Promise<DirectoryContent[]> => {
    const response = await axios.get<DirectoryContent[]>(
      apiRoutes.getDirectory(dirId)
    );
    return response.data;
  },

  fetchAllDirectories: async (): Promise<
    Map<string, Map<string, DirectoryContent>>
  > => {
    const response = await axios.get<DirectoryContentBatch>(
      apiRoutes.getDirectoriesBatch()
    );
