...
import { ExpandableTable, type ControTableRef, type DataSourceWithKey } from '../../../shared/ui/ExpandableTable';
import { IoTrash } from "react-icons/io5"
import { DirectoryValue } from '@shared/ui/DirectoryValue';
import { memo, useCallback, useMemo, useRef, useState, type MouseEventHandler, type MutableRefObject } from "react"
...
export const ArrayField: React.FC<ArrayFieldProps> = (props) =>
  <Field name={props.inputId}>
    {
      (formik: FieldProps<JsonObject[] | null>) =>
        <ArrayFieldInternal {...props} value={formik.field.value} />
    }
  </Field>;
    ...
    <div className={styles.fullRow}>
      <ExpandableCard labelProps={{
        ...
        <FieldArray name={inputId}>
          {
            arrayHelpers =>
              <ArrayTableControl fd={fd} inputId={inputId} value={value}
...
const ArrayTableControl = memo((props: ArrayTableControlProps) => {
  ...
  const dataSource = useMemo(() =>
    value?.map(x => ({ ...x, key: x[fd.propDescriptor.idPropertyName!] as string | number }))
    , [value, fd])

  const controTableRef = useRef<ControTableRef | null>(null);
  const [modal, setModal] = useState(modalConfirmClosedState)

  const onRemoveHandler = useCallback((index: number, key: Key, warn: boolean) => {
    if (warn) {
      setModal({
        open: true, content: <Text>В строке присутствуют данные, вы точно хотите её удалить?</Text>,
        onConfirm: () => { onRemoveHandler(index, key, false) }
      })
      return
    }
...
setTimeout(() => {
  document.querySelector<HTMLLabelElement>("#" + CSS.escape(`${inputId}[${idx}].${firstField.propDescriptor.id}-label`))
    ?.click()
})
...
if (table) {
  ...
  return (
    <div className={styles.cardContentContainer}>
      ...
      <ArrayTable controTableRef={controTableRef} onRemove={onRemoveHandler} dataSource={dataSource} inputId={inputId} fd={fd} />
      <ModalConfirm state={modal} setState={setModal} headerText="Внимание" mainButtonText="Удалить" mainButtonColor="error" />
    </div>
  );
})
...
type ArrayTableProps = {
  onRemove: (index: number, key: Key, warn: boolean) => void;
...
const ArrayTable = memo((
  ...
  const columns = useMemo(() => {
    const columns = fd.children!
      .filter(ffd => ffd.propDescriptor.type !== FieldType.Hidden)
      .map<ColumnProps<DataSource>>(x => {
        ...
        return {
          ...
          dataIndex: pd.id,
          title: pd.title,
          renderCellContent: pd.type === FieldType.Directory
...
columns.push({
  ...
  renderCellContent: ({ row, rowIndex }) =>
    <RemoveRowButton disabled={disabled} onClick={() => {
