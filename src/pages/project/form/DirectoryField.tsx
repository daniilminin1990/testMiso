...
import {
  DirectoriesContext,
  type Directory,
} from "@shared/contexts/directories/DirectoriesContext";
  ...
  return (
    <Field name={props.inputId}>
      {(formik: FieldProps<DirectoryFieldValue, JsonObject>) => {
        ...
        const options =
          pd.valuesFrom && directory
            ? getValuesFromOptions(formik, pd, directory)
            : directory;
...
function getValuesFromOptions(
  ...
  const valuesFrom =
    (getIn(formik.form.values, pd.valuesFrom!) as string[] | null) ?? [];
  const cur = formik.field.value ?? [];
  const map = new Map<string, DirectoryContentModel>(
    valuesFrom.map((x) => [x, directory.get(x)!] as const)
  );
  for (const v of cur)
    if (!map.has(v)) map.set(v, { ...directory.get(v)!, unavailable: true });
  return map;
}

function getFilteredItemsByInput(
  ...
  const matchingItems = values.filter(
    (x) =>
      x.label.toLowerCase().includes(input) ||
      x.code.toLowerCase().includes(input)
  );

  matchingItems.push(
    ...values.filter((directoryItem) =>
      matchingItems.some((item) =>
        directoryItem.code.startsWith(item.code.replace(/^0+/, ""))
      )
    )
  );
      ...
      filterOption: !directory
        ...
        : (option: DirectoryContentModel, inputValue: string) => {
            ...
            if (filterCacheRef.current.inputValue !== inputValue) {
              filterCacheRef.current = getFilteredItemsByInput(
                directory,
                inputValue
              );
            }
            return filterCacheRef.current.matchingItems.some(
  ...
  ({
    ...
    return (
      <ComboBox
        ...
        onChange={(v: DirectoryContentModel | null) =>
          setFieldValue(inputId, v?.value ?? null)
        }
      />
    );
  }
