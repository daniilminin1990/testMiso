...
import {
  ...
  type ControlEventTransitionDto,
...
export interface ControlEventFormProps {
  ...
  handleSubmit: (
    values: JsonObject,
    goToStatus: ControlEventTransitionDto | null
  ) => Promise<void>;
}
...
export const ControlEventForm: FC<ControlEventFormProps> = memo((props) => {
  ...
  const initialValues = useMemo(
    () =>
      data.fields.reduce<JsonObject>(
        (ac, x) => ((ac[x.descriptor.propDescriptor.id] = x.value), ac),
        {}
      ),
    [data]
  );
  ...
  const formSubmit = async (
    values: JsonObject,
    _formik: FormikHelpers<JsonObject>
  ) => {
...
const DynamicForm = memo((props: { data: ControlEventDto }): ReactNode => {
  ...
  const groups = useMemo(
    () =>
      Object.entries(
        undoPartial(
          Object.groupBy(data.fields, (x) => x.descriptor.propDescriptor.group)
        )
      ),
    [data]
  );
