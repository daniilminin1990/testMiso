...
import { ControlEventDto, EditControlEventCommand, type HttpValidationProblemDetails } from "../../shared/types/apiTypes";
import { apiRoutes } from "@shared/api/apiRoutes";

function isValidationProblem(data: unknown): data is HttpValidationProblemDetails {
  return typeof data === 'object' && !!data && 'type' in data && data.type === 'https://tools.ietf.org/html/rfc9110#section-15.5.1';
}
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        if (isValidationProblem(error.response.data))
          return { status: 'validation', errorData: error.response.data.errors ?? [] };
      }
